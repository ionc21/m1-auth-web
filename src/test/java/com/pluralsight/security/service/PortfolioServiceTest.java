...
@RunWith(MockitoJUnitRunner.class) public class PortfolioServiceTest {
    ...
    public void setup() {
        ...
        transactions.add(new Transaction(bitcoin, BUY, new BigDecimal("3.1"), new BigDecimal("15000.00"),
                System.currentTimeMillis()));
        transactions.add(new Transaction(litecoin, BUY, new BigDecimal("20.1"), new BigDecimal("200.00"),
                System.currentTimeMillis()));
        transactions.add(new Transaction(litecoin, BUY, new BigDecimal("200.1"), new BigDecimal("100.00"),
                System.currentTimeMillis()));
        transactions.add(new Transaction(litecoin, Type.SELL, new BigDecimal("201.1"), new BigDecimal("150.00"),
                System.currentTimeMillis()));
    ...
    public void testGetPortfolioPositions() {
        ...
        PositionDto position = repsonse
                .getPositionForCrypto(new CryptoCurrencyDto(bitcoin.getSymbol(), bitcoin.getName()));
    ...
    public void testAddAndDeleteTransaction() {
        Transaction transaction = new Transaction(bitcoin, BUY, new BigDecimal("3.6"), new BigDecimal("1500.00"),
                System.currentTimeMillis());
