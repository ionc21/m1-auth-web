...
public class PortfolioTest {
    ...
    @Test public void testAddTransaction() {
        //Given
        Portfolio portfolio = new Portfolio("snakamoto", new ArrayList<>(1));
        CryptoCurrency bitcoin = new CryptoCurrency("BTC", "Bitcoin");
        //When
        portfolio.addTransaction(new Transaction(bitcoin, BUY, new BigDecimal(3.1), new BigDecimal(15000.00),
                System.currentTimeMillis()));
        //Then
        assertEquals(1, portfolio.getTransactions().size());
    }

    @Test public void testGetAllTransactionsForCoin() {
        //Given
        ...
        transactions.add(new Transaction(bitcoin, BUY, new BigDecimal(3.1), new BigDecimal(15000.00),
                System.currentTimeMillis()));
        transactions.add(new Transaction(litecoin, BUY, new BigDecimal(20.1), new BigDecimal(13000.00),
                System.currentTimeMillis()));
        transactions.add(new Transaction(litecoin, BUY, new BigDecimal(200.1), new BigDecimal(130000.00),
                System.currentTimeMillis()));
        //When
    ...
    @Test(expected = NullPointerException.class) public void testUsernameIsMandatory() {
